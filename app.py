...
def edit_details(this_user):
    if request.method == "POST":
        ...
        pw = mongo.db.users.find_one({"username": session["user"]})["password"]
        edit_user = {
    ...
    user_id = mongo.db.users.find_one({"username": session["user"]})["_id"]
    this_record = mongo.db.users.find_one({"_id": ObjectId(user_id)})
...
def add_comment(book_id):
    if request.method == "POST":
        ...
        comments_by_len = len(mongo.db.books.find_one({"_id": ObjectId(book_id)})["comments_by"])
